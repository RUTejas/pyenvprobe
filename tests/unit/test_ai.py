...
def test_ai_generate_readme():
    with tempfile.TemporaryDirectory() as td:
        ...
        with (
            patch.dict(os.environ, {"PYENVPROBE_API_KEY": "dummy_key"}),
            patch("pyenvprobe.ai.OpenAI") as mock_openai,
        ):
            # Mock the response
            ...
            assert "TestProj" in (tmp_path / "README.md").read_text(encoding="utf-8")
...
def test_ai_generate_docstrings():
    with tempfile.TemporaryDirectory() as td:
        ...
        with (
            ...
            mock_response = MagicMock()
            mock_response.choices = [MagicMock()]
            mock_response.choices[0].message.content = '"""This is a test docstring."""'
            mock_client.chat.completions.create.return_value = mock_response
            mock_openai.return_value = mock_client
